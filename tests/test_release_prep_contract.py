...
def test_installable_package_metadata_and_quickstart_are_documented() -> None:
    ...
    assert 'name = "phantom-training"' in pyproject
    ...
    assert "python -m ruff check phantom_training tests" in workflow
    assert "python -m phantom_training.cli eval-judge-scenario --out" in combined
...
def test_ci_runs_release_prep_gate() -> None:
    ...
    assert "python -m phantom_training.cli demo-loop --out" in workflow
    assert "python -m phantom_training.cli backend-lifecycle --out" in workflow
    assert "python -m phantom_training.cli eval-judge-scenario --out" in workflow
